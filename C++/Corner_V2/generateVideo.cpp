...
int main() {
    ...
    cv::VideoWriter video("harris_response.avi", cv::VideoWriter::fourcc('X', 'V', 'I', 'D'), 120, cv::Size(nn_event.width, nn_event.height), false);
    ...
    for (int i = 0; i < nn_event.event_count; i++) {
        ...
        if (total_time >= 66000) {
            ...
            //cv::cornerHarris(sae_1, temp, blockSize, apertureSize, k);
            video.write(temp);
            //cv::Mat temp = sae_1;
