...
// g++ -std=c++11 -w -O2 -I /usr/local/Cellar/eigen/3.4.0_1/include/eigen3/ -I /usr/local/Cellar/opencv/4.5.5_1/include/opencv4/ -o generateVideo generateVideo.cpp luvharris.cpp -lopencv_core -lopencv_imgcodecs -lopencv_imgproc -lopencv_highgui -lopencv_ml -lopencv_video -lopencv_features2d -lopencv_videoio
#include <iostream>
...
int main() {
    ...
    cv::VideoWriter video("output.avi", cv::VideoWriter::fourcc('X', 'V', 'I', 'D'), 120, cv::Size(nn_event.width, nn_event.height), false);
    ...
    for (int i = 0; i < nn_event.event_count; i++) {
        ...
        if (total_time >= 66000) {
            //cv::Mat temp = generateLookup(sae_1, blockSize, apertureSize, k); // This is the corner response matrix
            //video.write(temp);
            //cv::Mat temp = sae_1;
